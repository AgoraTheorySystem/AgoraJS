...
// --- INÍCIO DA CORREÇÃO ---
// Adiciona uma função para garantir que o SweetAlert2 (Swal) esteja carregado
...
function ensureSwalIsLoaded() {
    ...
    if (swalPromise) {
        return swalPromise; // Carregamento em progresso
    }
    
    // Inicia o carregamento
    swalPromise = new Promise((resolve, reject) => {
        // Verifica se o script já existe (em alguns HTMLs)
        if (document.querySelector('script[src*="sweetalert2"]')) {
            // Se existe, apenas espera pelo window.Swal
            ...
            // Se não existe, carrega o script dinamicamente
            console.log("Carregando SweetAlert2 dinamicamente para o logout...");
            const script = document.createElement('script');
            script.src = "https://cdn.jsdelivr.net/npm/sweetalert2@11";
            script.onload = resolve;
            script.onerror = () => {
                console.error("Falha ao carregar SweetAlert2.");
                reject("Falha ao carregar SweetAlert2.");
            };
            document.head.appendChild(script);
            
            // Carrega o CSS também, para garantir
...
async function getTranslation(key) {
    // Tenta usar a função global de traducao.js se ela existir
    if (window.getTranslation) {
        return await window.getTranslation(key);
    }

    // Fallback se a função global não estiver pronta
    ...
    try {
        ...
        console.error('Erro ao buscar tradução (fallback):', error);
...
async function logout() {
    // --- INÍCIO DA CORREÇÃO ---
    // Espera o Swal carregar ANTES de usá-lo
    try {
        await ensureSwalIsLoaded();
    } catch (error) {
        console.error(error);
        // Fallback se o Swal falhar: confirmação nativa (menos bonita)
        if (confirm("Tem certeza que deseja sair? (Ocorreu um erro ao carregar a caixa de diálogo)")) {
            signOut(auth).then(() => {
                sessionStorage.clear();
                window.location.href = '/index.html';
            }).catch(error => console.error('Erro ao deslogar:', error));
        }
        return; // Interrompe a execução se o Swal falhou
    }
    // --- FIM DA CORREÇÃO ---
...
function setupLanguageSwitcher() {
    ...
    const updateLanguage = async (lang) => {
        try {
            ...
            // Usa a função global de traducao.js se disponível
            if (window.updateLanguage) {
                // await window.updateLanguage(lang); // Esta função já existe em traducao.js e faz tudo
            } else {
                // Fallback caso traducao.js não esteja carregado (improvável)
                const response = await fetch(`/i18n/${lang}.json`);
                if (!response.ok) {
    ...
    if (languageToggle) {
        languageToggle.addEventListener('click', () => {
            ...
            // Chama a função global de traducao.js (que deve estar carregada)
            if(window.updateLanguage) {
                window.updateLanguage(nextLang);
            } else {
                updateLanguage(nextLang); // Fallback
            }
        });
    }

    // A inicialização do idioma já é feita pelo traducao.js, 
    // mas garantimos que a bandeira do menu esteja correta.
    const savedLanguage = localStorage.getItem('selectedLanguage') || 'pt';
    if (languages[savedLanguage] && flagImage) {
        flagImage.src = languages[savedLanguage].flag;
    }
}
